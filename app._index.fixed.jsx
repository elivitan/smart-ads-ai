...
export const loader = async ({ request }) => {
  ...
  // Server-authoritative plan: DB first, cookie as fallback (for SSR flash prevention)
...
export default function Index() {
  const { products: dbProducts, planFromCookie, isPaidServer, shop: shopDomain, needsInitialSync, previewLanding, subscription: serverSubscription } = useLoaderData();
  ...
  const [scanCredits, setScanCreditsRaw] = useState(() => {
    // Server is source of truth; fallback to sessionStorage for client-only state
  ...
  useEffect(() => {
    ...
    async function fetchSpend() {
      ...
      } catch(e) { console.error("fetchSpend failed:", e); }
    }
    fetchSpend();
  ...
  if (hasScanAccess && !previewLanding) {
