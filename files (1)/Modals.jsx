...
export const OnboardModal = React.memo(function OnboardModal({
  onClose, onboardTab, setOnboardTab, onboardStep, setOnboardStep,
  selectedPlan, selectPlan, googleConnected, setGoogleConnected,
  scanCredits, setScanCredits, onLaunchChoice,
}) {
  return (
      ...
      <div className="modal onboard-modal" onClick={e=>e.stopPropagation()}>
        ...
        {onboardTab==="credits" ? (
          <div className="onboard-content">
            ...
            <div className="scan-credit-packages">
              {[{amt:10,price:"$9.99",per:"$0.99/scan"},{amt:50,price:"$34.99",per:"$0.70/scan",best:true},{amt:100,price:"$59.99",per:"$0.60/scan"}].map((pkg,i)=>(
                <div key={i} className={`scan-credit-pkg ${pkg.best?"scp-popular":""}`} onClick={()=>{setScanCredits(scanCredits+pkg.amt);onClose();}}>
          ...
          <div className="onboard-content">
            ...
            {googleConnected && <button className="btn-onboard" onClick={()=>{onClose();onLaunchChoice();}}>🚀 Start Scanning My Store</button>}
          </div>
        )}
      </div>
    </div>
...
export const BuyCreditsModal = React.memo(function BuyCreditsModal({ onClose, aiCredits, setAiCredits }) {
  return (
      ...
      <div className="credits-modal" onClick={e=>e.stopPropagation()}>
        ...
        <div className="credits-packages">
          {[{amt:50,price:"$4.99"},{amt:200,price:"$14.99",best:true},{amt:500,price:"$29.99"}].map((pkg,i)=>(
            <div key={i} className={`credit-pkg ${pkg.best?"credit-pkg-popular":""}`} onClick={()=>{setAiCredits(aiCredits+pkg.amt);onClose();}}>
