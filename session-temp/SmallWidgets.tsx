...
export const Counter = React.memo(function Counter({ end, dur = 1200, suffix = "" }: CounterProps) {
...
export const ScoreRing = React.memo(function ScoreRing({ score, size = 54 }: ScoreRingProps) {
...
export const Speedometer = React.memo(function Speedometer({ value, max, label, color = "#6366f1", size = 120 }: SpeedometerProps) {
  ...
  function ptXY(pcx: number, pcy: number, pr: number, deg: number) { const rad = deg * Math.PI / 180; return { x: pcx + pr * Math.cos(rad), y: pcy + pr * Math.sin(rad) }; }
