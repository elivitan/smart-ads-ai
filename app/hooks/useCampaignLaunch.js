...
const STEP_PROGRESS = {
  ...
  CREATING: { pct: 30, label: "Creating campaign in Google Ads...", icon: "🚀" },
  ...
  LINKING_CONVERSIONS: { pct: 90, label: "Setting up conversion tracking...", icon: "📊" },
...
export function useCampaignLaunch({ onSuccess, onError } = {}) {
    ...
    pollRef.current = setInterval(async () => {
      ...
      if (!mountedRef.current || attempts > POLL_MAX_ATTEMPTS) {
        ...
        if (mountedRef.current) {
          ...
          setError("Timed out waiting for campaign creation. Check Google Ads dashboard.");
        }
        return;
      }

      try {
        const res = await fetch(`/app/api/campaign-status?launchId=${jobId}`, {
        ...
        if (data.state === "ENABLED") {
          ...
          setCampaignId(data.campaignId || data.steps?.find(s => s.campaignId)?.campaignId);
          setProgress(100);
          clearLaunchState();
          onSuccess?.(data);
          ...
          setError(data.error || "Campaign partially created — some steps failed");
  ...
  const start = useCallback(async (payload) => {
    if (state === LAUNCH_STATES.SUBMITTING || state === LAUNCH_STATES.POLLING) return;
    ...
    try {
      ...
      if (data.launchId) {
        ...
        if (data.state === "ENABLED" && data.success) {
          ...
          clearLaunchState();
          onSuccess?.(data);
        } else if (data.state === "FAILED" || data.state === "FAILED_PARTIAL") {
          setState(data.state === "FAILED_PARTIAL" ? LAUNCH_STATES.PARTIAL_FAILURE : LAUNCH_STATES.FAILED);
          setError(data.error);
          setSteps(data.steps || []);
        } else {
      ...
      if (!mountedRef.current) return;
      setState(LAUNCH_STATES.FAILED);
      setError(err.name === "TimeoutError" ? "Request timed out. Campaign may still be creating — check back soon." : err.message);
    }
  ...
  // ── Derived ────────────────────────────────────────────────────────
  const isLoading = state === LAUNCH_STATES.SUBMITTING || state === LAUNCH_STATES.POLLING;
  const isFailed = state === LAUNCH_STATES.FAILED || state === LAUNCH_STATES.PARTIAL_FAILURE;
