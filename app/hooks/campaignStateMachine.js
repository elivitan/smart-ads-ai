...
export function useCampaignControl({ campaignId, onStatusChange } = {}) {
  ...
  const verifyWithGoogle = useCallback(
    async (expectedState, attempt = 0) => {
      ...
      try {
        ...
        if (data.success) {
          ...
          if (
            googleStatus === expectedState ||
            googleStatus === "paused" ||
            googleStatus === "removed" ||
        ...
        if (attempt < VERIFY_MAX_ATTEMPTS) {
          setTimeout(
            () => verifyWithGoogle(expectedState, attempt + 1),
            VERIFY_DELAY,
          );
        } else {
          transition(STATES.FAILED_TEMP, {
            error: "Google Ads didn't confirm the action. Try syncing.",
          });
        }
      } catch (err) {
        if (!mountedRef.current) return;
