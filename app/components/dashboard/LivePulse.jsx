...
const LivePulse = React.memo(function LivePulse({
  campaigns, impressionsBase, clicksBase,
  campaignId, realSpend, campaignControlStatus,
  confirmRemove, setConfirmRemove, onPause, onRemove
}) {
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      if (Math.random() > 0.45) {
        ...
        setTimeout(() => {
          setLastEvent(LIVE_PULSE_EVENTS[Math.floor(Math.random() * LIVE_PULSE_EVENTS.length)]);
  ...
  return (
    <div className="pulse-card">
      <div className="pulse-header-row">
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          {campaignId && campaignControlStatus !== "removed" && campaignControlStatus !== "paused" && (
            <>
              <button className="pulse-btn pulse-btn-pause" style={{ padding: "5px 12px", fontSize: 12 }}
                onClick={onPause} disabled={campaignControlStatus === "pausing" || campaignControlStatus === "removing"}>
                {campaignControlStatus === "pausing" ? "ג³" : "ג¸ Pause"}
              </button>
              <button className="pulse-btn pulse-btn-remove" style={{ padding: "5px 12px", fontSize: 12 }}
                onClick={() => setConfirmRemove(true)} disabled={campaignControlStatus === "pausing" || campaignControlStatus === "removing"}>
                {campaignControlStatus === "removing" ? "ג³" : "נ—‘ Remove"}
              </button>
            </>
          )}
          {campaignControlStatus === "paused" && <span className="pulse-status-badge pulse-badge-paused">ג¸ Paused</span>}
          {campaignControlStatus === "removed" && <span className="pulse-status-badge pulse-badge-removed">ג… Removed</span>}
      ...
      <div className="pulse-metrics-row">
        {[
          { val: impressions.toLocaleString(), lbl: "נ‘ Impressions", cls: "pulse-m-imp" },
          { val: clicks.toLocaleString(), lbl: "נ‘† Clicks", cls: "pulse-m-clk" },
            ...
            <span className="pulse-spend-val">
              {realSpend != null
                ...
                : (campaignControlStatus === "paused" || campaignControlStatus === "removed")
                  ? `$${spend}`
                  : <span className="pulse-spend-fetching">Fetching from Googleג€¦</span>}
            </span>
            {realSpend == null && <span className="pulse-spend-note">(estimated)</span>}
          </div>
          {campaignControlStatus === "error" && (
            <span className="pulse-status-badge pulse-badge-error">ג ן¸ Action failed ג€” check Google Ads connection</span>
          )}
          {confirmRemove && (
