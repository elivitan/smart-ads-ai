...
const StoreHealthScore = React.memo(function StoreHealthScore({
  analyzedCount, totalProducts, avgScore, highPotential, competitorCount,
  ...
  const decayFactor = useMemo(() => {
    ...
    const daysSinceScan = Math.max(0, (Date.now() - new Date(lastScanDate).getTime()) / 86400000);
  ...
  const vsIndustryLabel = vsIndustry > 0
  ...
  const staleMessage = decayFactor < 0.5
  ...
  const subScores = [
    { label: "Ad Quality", value: adQuality, color: "#6366f1", icon: "🎯",
      tip: `Avg score ${avgScore}/100 across products` },
    { label: "Product Coverage", value: productCoverage, color: "#06b6d4", icon: "📦",
      tip: `${analyzedCount}/${totalProducts} products analyzed` },
    { label: "Competitor Intel", value: competitorIntel, color: "#8b5cf6", icon: "🔍",
      tip: `${competitorCount} competitors found` },
    { label: "Budget Efficiency", value: budgetEfficiency, color: "#f59e0b", icon: "💰",
  ...
  return (
    <div className="health-card">
      <div className="health-card-header">
        <div className="health-card-title-row">
          ...
          <div>
            ...
            <div className="health-card-sub">AI-powered store readiness analysis</div>
          </div>
        </div>
        <div className="health-grade-circle" style={{ borderColor: gradeColor }}>
          <span className="health-grade-letter" style={{ color: gradeColor }}>{grade}</span>
        </div>
      </div>
      ...
      <div className="health-overall">
        <div className="health-score-ring">
          <svg width="90" height="90" viewBox="0 0 90 90">
            ...
            <circle cx="45" cy="45" r="38" fill="none" stroke={gradeColor} strokeWidth="6"
          ...
          <span className="health-score-number" style={{ color: gradeColor }}>{animated ? overall : 0}</span>
        </div>
        <div className="health-overall-info">
          <div className="health-status" style={{ color: gradeColor }}>{statusText}</div>
          <div className="health-industry-compare" style={{ color: vsIndustry >= 0 ? "#22c55e" : "#f59e0b", fontSize: 12, marginTop: 4 }}>
            {vsIndustryLabel}
          </div>
          <div style={{ display: "flex", gap: 8, marginTop: 6, fontSize: 11 }}>
            <span style={{ color: confidenceColor }}>Confidence: {confidenceLabel}</span>
      ...
      {/* Staleness warning */}
      {staleMessage && (
        <div style={{ background: "rgba(249,115,22,.1)", border: "1px solid rgba(249,115,22,.3)", borderRadius: 8, padding: "8px 12px", margin: "8px 0", fontSize: 12, color: "#f97316" }}>
          ⏰ {staleMessage}
        </div>
      )}
      ...
      {criticalPenalty > 0 && (
        <div style={{ background: "rgba(239,68,68,.1)", border: "1px solid rgba(239,68,68,.3)", borderRadius: 8, padding: "8px 12px", margin: "8px 0", fontSize: 12, color: "#ef4444" }}>
          ⚠ {criticalIssues} critical issue{criticalIssues > 1 ? "s" : ""} detected (−{criticalPenalty} points)
        </div>
      )}
          ...
          <div key={s.label} className="health-sub-row">
            ...
            <div className="health-sub-info">
              ...
              <div className="health-sub-bar-bg">
                <div className="health-sub-bar-fill"
                  style={{ width: animated ? `${s.value}%` : "0%", background: s.color, transition: "width 1s ease-out" }}
                />
              </div>
            </div>
            <span className="health-sub-val" style={{ color: s.color }}>{s.value}</span>
          </div>
        ))}
      </div>
      ...
      {expanded && (
        <div style={{ marginTop: 12, padding: "10px 12px", background: "rgba(255,255,255,.03)", borderRadius: 8, fontSize: 11, color: "rgba(255,255,255,.5)", lineHeight: 1.6 }}>
          <div><strong>Score breakdown:</strong></div>
          <div>Raw score: {rawOverall} → Decay ({Math.round(decayFactor * 100)}%): {decayedScore}
            {criticalPenalty > 0 ? ` → Penalty: −${criticalPenalty}` : ""}
            → Confidence ({Math.round(confidence * 100)}%): <strong style={{ color: gradeColor }}>{overall}</strong>
          </div>
          <div style={{ marginTop: 4 }}>Industry baseline: {baseline} | Your score: {overall} ({vsIndustryLabel})</div>
          {subScores.map(s => (
            <div key={s.label}>{s.icon} {s.label}: {s.value}/100 — {s.tip}</div>
          ))}
        </div>
      )}

      <button className="health-expand-btn" onClick={() => setExpanded(!expanded)}>
