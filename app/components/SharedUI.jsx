...
export const Counter = React.memo(function Counter({ end, dur = 1200, suffix = "" }) {
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      start += step;
      if (start >= end) { setV(end); clearInterval(timer); }
      else setV(start);
    }, 16);
  ...
  return (
    <svg width={size} height={size}>
      ...
      <text x="50%" y="50%" dominantBaseline="central" textAnchor="middle"
        fill={color} fontSize={size * 0.28} fontWeight="800">{score}</text>
    </svg>
  );
}

export const Speedometer = React.memo(function Speedometer({ value, max, label, color = "#6366f1", size = 130 }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
      <svg width={size} height={size / 2 + 10} viewBox={`0 0 ${size} ${size / 2 + 10}`}>
        <path d={`M 5 ${size / 2 + 5} A ${r} ${r} 0 0 1 ${size - 5} ${size / 2 + 5}`}
          fill="none" stroke="rgba(255,255,255,.07)" strokeWidth="8" strokeLinecap="round" />
        <path d={`M 5 ${size / 2 + 5} A ${r} ${r} 0 0 1 ${size - 5} ${size / 2 + 5}`}
          fill="none" stroke={color} strokeWidth="8" strokeLinecap="round"
          strokeDasharray={c} strokeDashoffset={offset}
          style={{ transition: "stroke-dashoffset 1.2s cubic-bezier(.4,0,.2,1)" }} />
        <text x="50%" y={size / 2 - 4} dominantBaseline="central" textAnchor="middle"
          fill="#fff" fontSize="22" fontWeight="800">{value}</text>
      </svg>
  ...
  const pieces = Array.from({ length: 60 }, (_, i) => {
    ...
    return (
      <div key={i} style={{
        ...
        width: sz, height: sz * .4,
        background: colors[i % colors.length], borderRadius: 2,
        zIndex: 9999, transform: `rotate(${rot}deg)`,
        animation: `confettiFall ${dur}s ease-out ${delay}s forwards`, opacity: 0,
      }} />
    );
  });
