...
const LaunchProgress = React.memo(function LaunchProgress({ launch }) {
  const { state, steps, error, progress, stepInfo, campaignId, isLoading, isFailed, isPartial, isSuccess } = launch;

  const ALL_STEPS = [
    "QUEUED", "VALIDATING", "CREATING", "CAMPAIGN_CREATED",
  ...
  return (
    <div style={{
      ...
      <div style={{ textAlign: "center", marginBottom: 20 }}>
        ...
        <h3 style={{ margin: 0, fontSize: 18, color: isSuccess ? "#22c55e" : isFailed ? "#ef4444" : "#818cf8" }}>
          {isSuccess ? "Campaign is Live!" : isFailed ? "Launch Failed" : "Creating Your Campaign..."}
        </h3>
      ...
      <div style={{
        ...
        <div style={{
          ...
          borderRadius: 8,
          width: `${progress}%`,
          background: isSuccess ? "#22c55e" : isFailed ? "#ef4444" : "linear-gradient(90deg, #6366f1, #818cf8)",
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
        {ALL_STEPS.map((stepName) => {
          ...
          const failedStep = steps.find(s => s.state === "FAILED" || s.state === "FAILED_PARTIAL");
          const isFailedStep = failedStep && !isDone && stepName === ALL_STEPS[ALL_STEPS.indexOf(launch.currentStep) + 1];
          ...
          return (
            <div key={stepName} style={{
              ...
              padding: "6px 10px",
              borderRadius: 8,
              background: isCurrent ? "rgba(99,102,241,.1)" : isDone ? "rgba(34,197,94,.05)" : "transparent",
              ...
              <span style={{
                fontSize: 13,
                color: isDone ? "#22c55e" : isCurrent ? "#818cf8" : "rgba(255,255,255,.4)",
              ...
              {isDone && steps.find(s => s.state === stepName) && (
                <span style={{ fontSize: 10, color: "rgba(255,255,255,.2)", marginLeft: "auto" }}>
                  {new Date(steps.find(s => s.state === stepName).ts || steps.find(s => s.state === stepName).timestamp).toLocaleTimeString()}
                </span>
              )}
            </div>
          );
      ...
      {isFailed && (
        <div style={{
          ...
          padding: 12,
          borderRadius: 10,
          background: isPartial ? "rgba(245,158,11,.1)" : "rgba(239,68,68,.1)",
          border: `1px solid ${isPartial ? "rgba(245,158,11,.3)" : "rgba(239,68,68,.3)"}`,
        }}>
          <div style={{ fontSize: 13, color: isPartial ? "#f59e0b" : "#ef4444", marginBottom: 8 }}>
            {isPartial ? "⚠️ Campaign was partially created" : "❌ "}{error}
          </div>
          {isPartial && campaignId && (
            <div style={{ fontSize: 11, color: "rgba(255,255,255,.4)", marginBottom: 8 }}>
              Campaign {campaignId} was created but some steps failed. You can retry or manage it in Google Ads.
            </div>
          )}
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={launch.retry} style={{
              background: "rgba(99,102,241,.2)", border: "1px solid rgba(99,102,241,.4)",
              borderRadius: 8, padding: "6px 16px", color: "#818cf8", cursor: "pointer",
              fontSize: 12, fontWeight: 600,
            }}>
              🔄 Retry
            </button>
            <button onClick={launch.reset} style={{
              background: "rgba(255,255,255,.05)", border: "1px solid rgba(255,255,255,.1)",
              borderRadius: 8, padding: "6px 16px", color: "rgba(255,255,255,.4)", cursor: "pointer",
              fontSize: 12,
            }}>
              Start Over
            </button>
      ...
      {isSuccess && (
        <div style={{
          ...
          <button onClick={launch.reset} style={{
            ...
            borderRadius: 8, padding: "8px 20px", color: "#22c55e", cursor: "pointer",
