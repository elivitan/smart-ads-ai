...
const Speedometer = React.memo(function Speedometer({
  ...
  const cx = svgW / 2,
    cy = svgH * 0.58,
    r = size * 0.34;
  function ptXY(pcx, pcy, pr, deg) {
    const rad = (deg * Math.PI) / 180;
    return { x: pcx + pr * Math.cos(rad), y: pcy + pr * Math.sin(rad) };
  }
  const arcStart = ptXY(cx, cy, r, startAngle),
    arcEnd = ptXY(cx, cy, r, endAngle),
    fillEnd = ptXY(cx, cy, r, angle);
    ...
    >
      <svg width={svgW} height={svgH} viewBox={`0 0 ${svgW} ${svgH}`}>
        ...
        <path
          ...
          strokeWidth="6"
          strokeLinecap="round"
          style={{ transition: "d 1s cubic-bezier(.4,0,.2,1)" }}
        />
        <line
          x1={cx}
          y1={cy}
