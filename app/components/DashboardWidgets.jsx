...
const StoreHealthScore = React.memo(function StoreHealthScore({
  ...
  const productCoverage =
    totalProducts > 0 ? Math.round((analyzedCount / totalProducts) * 100) : 0;
  const competitorIntel = Math.min(competitorCount * 20, 100);
  const budgetEfficiency =
    avgScore > 0
      ? Math.min(Math.round(avgScore * 0.85 + highPotential * 2.5), 100)
      : 0;
  ...
  const subScores = [
    ...
    },
    {
      ...
      color: "#06b6d4",
      icon: "📦",
      tip: `${analyzedCount} of ${totalProducts} analyzed`,
    },
    {
    ...
    },
    {
      ...
      color: "#f59e0b",
      icon: "💰",
      tip: "Estimated ROI based on scores",
    },
  ];
    ...
    <div className="health-card" onClick={() => setExpanded((e) => !e)}>
      <div className="health-top">
        ...
        <div className="health-ring-wrap">
          <svg width={sz} height={sz}>
            ...
            <circle
              ...
              style={{
                transition:
                  "stroke-dashoffset 1.6s cubic-bezier(.4,0,.2,1), stroke .5s",
              }}
            />
            <text
              x="50%"
          ...
          />
          <div
            className="health-pulse health-pulse-2"
            style={{ borderColor: `${gradeColor}15` }}
          />
        </div>
        ...
        <div className="health-info">
          ...
          <div className="health-status-text" style={{ color: gradeColor }}>
            {statusText}
          </div>
                ...
                <div className="health-mini-track">
                  <div
                    ...
                    style={{
                      ...
                      background: s.color,
                      transition: `width ${1.2 + i * 0.15}s cubic-bezier(.4,0,.2,1)`,
                    }}
                  />
                </div>
                <span className="health-mini-val" style={{ color: s.color }}>
                  {s.value}
                </span>
              </div>
            ))}
          </div>
          <div className="health-expand">
              ...
              <div key={i} className="health-sub-item">
                <svg width="52" height="52">
                  ...
                  <circle
                    ...
                    style={{
                      transition: `stroke-dashoffset ${1.2 + i * 0.2}s cubic-bezier(.4,0,.2,1)`,
          ...
          <div className="health-tips">
            {productCoverage < 100 && (
              <div className="health-tip-item">
                💡 Analyze {totalProducts - analyzedCount} more products to
                improve coverage
              </div>
            )}
            {adQuality < 70 && (
              <div className="health-tip-item">
                💡 Boost low-scoring products with stronger keywords
              </div>
            )}
            {competitorIntel < 60 && (
              <div className="health-tip-item">
                💡 Run full scan to gather more competitor intelligence
              </div>
            )}
          </div>
        </div>
...
const LivePulse = React.memo(function LivePulse({
  ...
  return (
    <div className="pulse-card">
      ...
      >
        {campaigns} campaign{campaigns !== 1 ? "s" : ""} running · live data
      </div>
        ...
        <div className="pulse-controls">
          ...
          {campaignControlStatus === "error" && (
            <span className="pulse-status-badge pulse-badge-error">
              ⚠️ Action failed — check Google Ads connection
            </span>
          )}
...
const TopMissedOpportunity = React.memo(function TopMissedOpportunity({
  topProduct,
  avgScore,
  totalMonthlyGapLoss,
  analyzedCount,
    ...
    return (
      <div className="tmo-card tmo-teaser">
        ...
        <div className="tmo-teaser-content">
          <h3 className="tmo-teaser-title">
            Discover Your #1 Missed Opportunity
          </h3>
          <p className="tmo-teaser-sub">
            Run a free scan to see which product could be making you the most
            money right now — and exactly why it isn't.
          </p>
          <button className="tmo-teaser-btn" onClick={onScan}>
            ⚡ Run Free Scan Now
          </button>
        </div>
  ...
  const topKeyword =
    ai.keywords?.[0]?.text || ai.keywords?.[0] || "your top keyword";
  const estMonthly =
    totalMonthlyGapLoss > 0
      ? totalMonthlyGapLoss
      : Math.round(score * 18 + 240);
    ...
    <div className="tmo-card">
      ...
      <div className="tmo-content">
        <div className="tmo-left">
          {topProduct.image && (
            <img src={topProduct.image} alt="" className="tmo-img" />
          )}
          <div className="tmo-product-info">
            <div className="tmo-product-title">{topProduct.title}</div>
            <div className="tmo-product-price">
              ${Number(topProduct.price || 0).toFixed(2)}
            </div>
            <div className="tmo-score-row">
              <div className="tmo-score-bar">
                <div
                  className="tmo-score-fill"
                  style={{
                    width: `${score}%`,
                    background:
                      score >= 70
        ...
        <div className="tmo-right">
          ...
          <div className="tmo-insights">
            {topComp && (
              <div className="tmo-insight">
                ⚔️ <strong>{topComp}</strong> is bidding on "
                <em>{topKeyword}</em>" right now
              </div>
            )}
            <div className="tmo-insight">
              🔑 Top keyword: <strong>"{topKeyword}"</strong>
            </div>
            {ai.strategy && (
              <div className="tmo-insight">
                📋 Recommended strategy:{" "}
                <strong>{ai.strategy.replace(/_/g, " ").toUpperCase()}</strong>
              </div>
            )}
          </div>
          <button
            className="tmo-cta"
            onClick={() => onViewProduct && onViewProduct(topProduct)}
          >
            🚀 View Full Campaign →
          </button>
        </div>
...
const BudgetSimulator = React.memo(function BudgetSimulator({
  avgScore,
  avgCpc,
  canPublish,
  onUpgrade,
  ...
  const breakEvenDays =
    dailyProfit > 0 ? Math.ceil(monthlyBudget / dailyProfit) : null;
  const roasNum = parseFloat(roas);
  const roasColor =
    roasNum >= 4 ? "#22c55e" : roasNum >= 2 ? "#f59e0b" : "#ef4444";
    ...
    <div className="budget-sim-card">
      <div className="budget-sim-header">
        <div>
          ...
          <p className="budget-sim-sub">
            Adjust your budget and see projected results
          </p>
        </div>
        {!canPublish && (
          <button className="budget-sim-upgrade" onClick={onUpgrade}>
            🔒 Subscribe to Launch
          </button>
        )}
      </div>
      ...
      <div className="budget-sim-inputs">
        <div className="budget-sim-input-row">
          ...
          <input
            ...
            value={vals.budget}
            onChange={(e) =>
              setVals((v) => ({ ...v, budget: Number(e.target.value) }))
            }
        ...
        <div className="budget-sim-input-row">
          ...
          <input
            ...
            value={vals.aov}
            onChange={(e) =>
              setVals((v) => ({ ...v, aov: Number(e.target.value) }))
            }
            className="budget-sim-slider"
          />
          <div className="budget-sim-range-labels">
        ...
        <div className="budget-sim-input-row">
          ...
          <input
            ...
            value={vals.conv}
            onChange={(e) =>
              setVals((v) => ({ ...v, conv: parseFloat(e.target.value) }))
            }
            className="budget-sim-slider"
          />
          <div className="budget-sim-range-labels">
      ...
      <div className="budget-sim-results">
        <div className="budget-sim-result-card">
          <div className="budget-sim-result-val">
        ...
        <div
          className="budget-sim-result-card"
          style={{ borderColor: roasColor + "44" }}
        >
          <div className="budget-sim-result-val" style={{ color: roasColor }}>
            {roas}x
          </div>
          <div className="budget-sim-result-lbl">
            📈 ROAS{" "}
            <span style={{ color: roasColor, fontSize: 10 }}>
              ({roasLabel})
            </span>
          </div>
        </div>
      ...
      <div className="budget-sim-monthly">
        <div className="budget-sim-monthly-row">
          ...
          <span style={{ color: "#ef4444" }}>
            -${(vals.budget * 30).toLocaleString()}
          </span>
        </div>
        <div className="budget-sim-monthly-row">
          <span>Monthly revenue</span>
          <span style={{ color: "#22c55e" }}>
            +${Math.round(dailyRevenue * 30).toLocaleString()}
          </span>
        </div>
        <div
          className="budget-sim-monthly-row"
          style={{
            fontWeight: 800,
            fontSize: 15,
        ...
        >
          ...
          <span style={{ color: dailyProfit >= 0 ? "#22c55e" : "#ef4444" }}>
            {dailyProfit >= 0 ? "+" : ""}$
            {Math.round((dailyRevenue - vals.budget) * 30).toLocaleString()}
          </span>
        </div>
      ...
      <div className="budget-sim-note">
        * Based on avg score {avgScore}/100 · Est. CPC ${cpc.toFixed(2)} ·
        Results may vary
      </div>
    </div>
  );
