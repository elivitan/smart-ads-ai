...
export function ToastProvider({ children }) {
  ...
  const error = useCallback((msg, dur) => addToast(msg, "error", dur || 8000), [addToast]);
  const warning = useCallback((msg, dur) => addToast(msg, "warning", dur || 6000), [addToast]);
...
function Toast({ toast, onDismiss }) {
  ...
  const colors = {
    success: { bg: "rgba(34,197,94,.15)", border: "rgba(34,197,94,.4)", icon: "ג…", text: "#22c55e" },
    error: { bg: "rgba(239,68,68,.15)", border: "rgba(239,68,68,.4)", icon: "ג", text: "#ef4444" },
    warning: { bg: "rgba(245,158,11,.15)", border: "rgba(245,158,11,.4)", icon: "ג ן¸", text: "#f59e0b" },
    info: { bg: "rgba(99,102,241,.15)", border: "rgba(99,102,241,.4)", icon: "ג„¹ן¸", text: "#6366f1" },
  };
  ...
  return (
    <div style={{
      ...
      <span style={{ color: c.text, fontSize: 13, fontWeight: 500, lineHeight: 1.4, flex: 1 }}>
        {toast.message}
      </span>
      <span style={{ color: "rgba(255,255,255,.3)", fontSize: 16, marginLeft: 4 }}>ֳ—</span>
    </div>
  );
}
      ...
      return (
        <div style={{
          ...
          {process.env.NODE_ENV === "development" && this.state.errorInfo && (
            <details style={{ marginTop: 16, textAlign: "left", fontSize: 11, color: "rgba(255,255,255,.3)" }}>
        ...
        <div style={{
          ...
          <div style={{ display: "flex", gap: 10, justifyContent: "center" }}>
            <button onClick={reset} style={{
              ...
              borderRadius: 8, padding: "8px 20px", color: "#6366f1", cursor: "pointer", fontSize: 13, fontWeight: 600,
            }}>
              Retry Wizard
            </button>
            {onClose && (
              <button onClick={onClose} style={{
                ...
                borderRadius: 8, padding: "8px 20px", color: "rgba(255,255,255,.5)", cursor: "pointer", fontSize: 13,
              }}>
                Close
              </button>
            )}
