...
// Shows actionable insights, NOT raw data (that's the dashboard's job)
// PROPS: all real data from Index(), plus onOpenDashboard callback
...
function MiniSpark({ data, color, h = 28 }) {
  ...
  const max = Math.max(...data), min = Math.min(...data), range = max - min || 1, w = 64;
  const pts = data.map((v, i) => `${(i / (data.length - 1)) * w},${h - ((v - min) / range) * (h - 4) - 2}`).join(" ");
  return (
    <svg width={w} height={h} style={{ display: "block" }}>
      <polyline points={pts} fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" opacity=".5" />
    </svg>
  );
}

function IconBox({ children, bg, border }) {
  return (
    <div style={{
      width: 36, height: 36, borderRadius: 10, flexShrink: 0,
      background: bg || "rgba(109,40,217,.08)",
      border: `1px solid ${border || "rgba(109,40,217,.15)"}`,
...
export function SubscriberHome({
  ...
  let aiTip = "";
  if (avgScore < 50) {
    aiTip = `Your average score is ${avgScore}/100 — most products need optimization. Running a full scan on all ${totalProducts} products will identify quick wins.`;
  } else if (gapCount > 5) {
    aiTip = `Found ${gapCount} keyword gaps where competitors rank and you don't. Closing the top 3 gaps could recover ~$${Math.round(gapLoss * 0.4).toLocaleString()}/mo in lost traffic.`;
  } else if (highPotential > 3) {
    aiTip = `You have ${highPotential} high-potential products scoring 70+. Launching campaigns for the top 5 could generate an estimated ${mockRoas}x ROAS.`;
  } else {
    aiTip = `Your store is performing well with an avg score of ${avgScore}/100. Consider increasing budget on your top products to capture more market share.`;
  }

  // Urgent actions based on real data
  const alerts = [];
  ...
  if (alerts.length === 0) {
    alerts.push({ t: "All systems running smoothly", s: "good", a: "Dashboard" });
  }

  const alertIcons = { bad: <AlertCircle size={14} />, warn: <AlertTriangle size={14} />, good: <Rocket size={14} /> };
  const alertStyles = {
    bad: { bg: "#fef2f2", border: "#fee2e2", dot: "#ef4444", text: "#991b1b", tag: "#fecaca", iconColor: "#ef4444" },
    warn: { bg: "#fffbeb", border: "#fef3c7", dot: "#f59e0b", text: "#92400e", tag: "#fde68a", iconColor: "#f59e0b" },
    good: { bg: "#f0fdf4", border: "#dcfce7", dot: "#22c55e", text: "#166534", tag: "#bbf7d0", iconColor: "#22c55e" },
  };

  // ── CSS (injected once) ──
  const css = `
    @import url('https://fonts.googleapis.com/css2?family=DM+Sans:opsz,wght@9..40,300;9..40,400;9..40,500;9..40,600;9..40,700;9..40,800&display=swap');
    .sh-root{min-height:100vh;background:linear-gradient(165deg,#fafbfe 0%,#f1f4fb 40%,#f6f4fb 70%,#faf8fc 100%);font-family:'DM Sans',system-ui,sans-serif;color:#1e293b}
    .sh-ambient{position:fixed;inset:0;pointer-events:none;z-index:0;background:radial-gradient(ellipse at 0% 0%,rgba(99,102,241,.04),transparent 40%),radial-gradient(ellipse at 100% 100%,rgba(168,85,247,.03),transparent 40%)}
    .sh-wrap{position:relative;z-index:1;max-width:1060px;margin:0 auto;padding:32px 28px 48px}
    .sh-btn{border:none;cursor:pointer;font-family:inherit;display:flex;align-items:center;gap:6px}
    .sh-btn-secondary{background:#fff;border:1px solid #e2e8f0;color:#475569;padding:10px 18px;border-radius:10px;font-size:13px;font-weight:600;box-shadow:0 1px 3px rgba(0,0,0,.04)}
    .sh-btn-primary{background:linear-gradient(135deg,#7c3aed,#6d28d9);color:#fff;padding:10px 20px;border-radius:10px;font-size:13px;font-weight:700;box-shadow:0 4px 14px rgba(109,40,217,.25)}
    .sh-card{background:#fff;border:1px solid #e8eaef;border-radius:14px;box-shadow:0 1px 3px rgba(0,0,0,.03),0 1px 2px rgba(0,0,0,.02);position:relative;overflow:hidden}
    .sh-card-lg{border-radius:16px;padding:20px 22px}
    .sh-product-row{display:flex;align-items:center;gap:14px;padding:12px 14px;margin-bottom:8px;background:#fafbfc;border-radius:10px;border:1px solid #f1f5f9;cursor:pointer;transition:background .15s}
    .sh-product-row:hover{background:#f1f5f9}
  `;
      ...
      <div className="sh-wrap">
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 12, marginBottom: 22 }}>
          {[
            ...
            <div key={i} className="sh-card" style={{ padding: "16px 16px 12px", ...anim(220 + i * 50) }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 22 }}>
          ...
          {/* Left: Top Products */}
          <div className="sh-card sh-card-lg" style={anim(480)}>
            ...
            {topProducts.length > 0 ? topProducts.map((p, i) => {
              ...
              return (
                <div key={i} className="sh-product-row">
              ...
              <div style={{ textAlign: "center", padding: "24px 0", color: "#94a3b8", fontSize: 13 }}>
                <Package size={24} style={{ marginBottom: 8, opacity: .4 }} />
                <div>Run a scan to see your top products</div>
              </div>
            )}
          ...
          <div className="sh-card sh-card-lg" style={anim(560)}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: 14 }}>
              {[
                ...
                <div key={i} style={{ background: m.bg, border: `1px solid ${m.border}`, borderRadius: 10, padding: "12px", textAlign: "center" }}>
              ...
              <>
                ...
                {topCompetitors.slice(0, 3).map((comp, i) => (
                  <div key={i} style={{
                    ...
                    <IconBox bg="rgba(220,38,38,.05)" border="rgba(220,38,38,.1)">
        ...
        <div onClick={onOpenDashboard} style={{
          ...
          <button className="sh-btn sh-btn-primary" style={{ padding: "12px 24px", fontSize: 14 }}>
