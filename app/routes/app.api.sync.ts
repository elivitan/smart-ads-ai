...
const _action = async ({ request }: RouteHandlerArgs): Promise<Response> => {
  ...
  try {
    ...
    logger.error("sync.action", "Auth failed", { error: (authErr as Error).message });
  ...
  try {
    ...
    if (step === "analyze_batch") {
      const batchSize = parseInt((formData.get("batchSize") as string) || "10", 10);
    ...
    if (step === "analyze_parallel") {
      const batchSize = parseInt((formData.get("batchSize") as string) || "5", 10);
      const parallelCount = parseInt((formData.get("parallel") as string) || "3", 10);
      const force = formData.get("force") === "true";
    ...
    logger.error("sync.action", "Sync API error", { shop, error: (err as Error).message });
    return Response.json({ success: false, error: (err as Error).message || "Sync failed" }, { status: 500 });
  }
};
