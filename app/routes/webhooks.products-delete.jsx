...
/**  * Webhook: products/delete  *   * Shopify sends this when a product is deleted.  * We remove it from the local DB.  */ import { authenticate } from "../shopify.server.js";
import { handleProductDelete } from "../sync.server.js";
export const action = async ({ request }) => {
  const { shop, payload } = await authenticate.webhook(request);
  console.log(`Webhook: products/delete for shop ${shop}`);
  if (!payload || !payload.id) {
    return json("No payload", { status: 200 });
  }
  try {
