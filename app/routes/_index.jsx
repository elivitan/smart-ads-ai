...
// ── Custom Hooks (extracted logic — safe to edit without touching UI) ──
...
export const loader = async ({ request }) => {
  ...
  const analyzedCount = dbProducts.filter(p => p.hasAiAnalysis).length;
  return {
    products: dbProducts,
    syncStatus,
    shop,
      ...
      <div className="budget-sim-inputs">
        <div className="budget-sim-input-row">
          ...
          <input type="range" min="5" max="500" step="5" value={vals.budget}
            onChange={e => { const val = Number(e.target.value); setVals(v => ({...v, budget: val})); }}
            className="budget-sim-slider" />
        ...
        <div className="budget-sim-input-row">
          ...
          <input type="range" min="10" max="500" step="5" value={vals.aov}
            onChange={e => { const val = Number(e.target.value); setVals(v => ({...v, aov: val})); }}
            className="budget-sim-slider" />
        ...
        <div className="budget-sim-input-row">
          ...
          <input type="range" min="0.1" max="10" step="0.1" value={vals.conv}
            onChange={e => { const val = parseFloat(e.target.value); setVals(v => ({...v, conv: val})); }}
            className="budget-sim-slider" />
...
export default function Index() {
  const {
    ...
    shop: shopDomain,
    needsInitialSync,
    isScannedOnce: serverIsScannedOnce,
  } = useLoaderData();
  ...
  // ══════════════════════════════════════════════════════════════════════════
  //  HOOKS — all declared unconditionally
  // ══════════════════════════════════════════════════════════════════════════

  // -- Subscription & access rights --
  const {
    selectedPlan, scanCredits, aiCredits, isHydrated,
    isPaid, hasScanAccess, canPublish,
    selectPlan, setScanCredits, setAiCredits,
  } = useSubscription({ isPaidServer, planFromCookie });

  // -- UI / modal state --
  const {
    vis, showOnboard, onboardStep, onboardTab,
    showBuyCredits, showManualPicker, pickedProducts,
    showConfetti, showCancelConfirm, selCompetitor,
    selProduct, editHeadlines, editDescriptions, improvingIdx,
    setShowOnboard, setOnboardStep, setOnboardTab,
    setShowBuyCredits, setShowManualPicker, setPickedProducts,
    setShowCancelConfirm, setSelCompetitor, setSelProduct,
    setEditHeadlines, setEditDescriptions, setImprovingIdx,
    triggerConfetti, openUpgrade,
  } = useUI();

  // -- Scanner --
  const {
    products, aiResults, isScanning, fakeProgress,
    scanMode, scanError, scanMsg, scanned,
    doScan, cancelScan, setAiResults, setScanError,
    cancelRef, creepRef,
  } = useScanner({
    hasScanAccess,
    canPublish,
    ...
    getProductUrl,
    triggerConfetti,
    onScanComplete: (_allAiProducts) => { /* nothing extra needed yet */ },
    onAutoLaunch: async (fetchedProducts, allAiProducts) => {
      setAutoLaunching(true);
      let sc = 0;
      for (let i = 0; i < fetchedProducts.length; i++) {
        const prod = fetchedProducts[i];
        const ai = (allAiProducts.find(ap => ap.title === prod.title || ap.id === prod.id)) || allAiProducts[i] || {};
        try {
      ...
      setAutoStatus(sc > 0 ? "success" : "error");
    },
  });

  // -- Campaigns --
  const {
    campaignId, campaignStatus, campaignControlStatus,
    realSpend, confirmRemove, autoLaunching, autoStatus,
    showLaunchChoice, googleConnected,
    setCampaignStatus, setCampaignControlStatus,
    setConfirmRemove, setAutoStatus, setAutoLaunching,
    setShowLaunchChoice, setGoogleConnected,
    handleAutoCampaign: handleAutoCampaignBase,
  ...
  const _as = _ac > 0
    ? Math.round(analyzedDbProducts.reduce((a, p) => a + (p.aiAnalysis?.ad_score || 0), 0) / _ac)
    : 0;
  ...
  const topCompetitors = useMemo(() => {
    const allC = analyzedDbProducts.flatMap(p => p.aiAnalysis?.competitor_intel?.top_competitors || []);
    const map = {};
    allC.forEach(c => {
      if (!c.domain) return;
      if (!map[c.domain]) map[c.domain] = { count: 0, strength: c.strength || "unknown" };
      map[c.domain].count++;
    });
  ...
  const { keywordGaps, totalMonthlyGapLoss } = useMemo(() => {
    const myKw = new Set(
      analyzedDbProducts
        .flatMap(p => (p.aiAnalysis?.keywords || []).map(k => (typeof k === "string" ? k : k?.text || "").toLowerCase().trim()))
        .filter(Boolean)
    );
    const compKw = analyzedDbProducts
      .flatMap(p => p.aiAnalysis?.competitor_intel?.keyword_gaps || [])
      .map(k => (typeof k === "string" ? k : k?.text || k).toLowerCase().trim())
      .filter(Boolean);
    const counts = {};
    compKw.forEach(k => { counts[k] = (counts[k] || 0) + 1; });
    const gaps = Object.entries(counts)
      .filter(([k]) => !myKw.has(k) && k.length > 3)
      .sort((a, b) => b[1] - a[1])
      .slice(0, 8)
      .map(([keyword, freq]) => ({
        keyword, freq,
  ...
  const handleProductClickRef   = useRef(null);
  const handleAutoCampaignRef   = useRef(null);
  useEffect(() => {
    handleProductClickRef.current = (product) => {
      if (!hasScanAccess) { openUpgrade(); return; }
      setSelProduct(product);
      setCampaignStatus(null);
      const ai = product.hasAiAnalysis
        ? (product.aiAnalysis || {})
        : (aiResults?.products?.find(ap => ap.title === product.title || ap.id === product.id) || {});
    ...
    handleAutoCampaignRef.current = () => {
      if (!canPublish) { openUpgrade(); return; }
      const toProcess = analyzedDbProducts.length > 0 ? analyzedDbProducts : allDbProducts.slice(0, 5);
      handleAutoCampaignBase(toProcess, getProductUrl);
    };
  });
  ...
  // handleProductClick / handleAutoCampaign aliases (referenced in render)
  function handleProductClick(product)  { handleProductClickRef.current(product); }
  function handleAutoCampaign()         { handleAutoCampaignRef.current(); }

  // ── handleCreateCampaign (needs selProduct + editHeadlines/Descriptions) ──
  async function handleCreateCampaign() {
    if (!selProduct || !canPublish) return;
    setCampaignStatus("creating");
    try {
      const ai = selProduct.hasAiAnalysis
        ? (selProduct.aiAnalysis || {})
        : (aiResults?.products?.find(ap => ap.title === selProduct.title) || {});
      const form = new FormData();
      form.append("productTitle",  selProduct.title);
      form.append("headlines",     JSON.stringify(editHeadlines));
      form.append("descriptions",  JSON.stringify(editDescriptions));
      form.append("keywords",      JSON.stringify(ai.keywords || []));
      form.append("finalUrl",      getProductUrl(selProduct));
      form.append("dailyBudget",   "50");
      const res  = await fetch("/app/api/campaign", { method: "POST", body: form });
      const data = await res.json();
      setCampaignStatus(data.success ? "success" : "error");
      if (data.success) {
        triggerConfetti();
        const cid = data.campaignId || data.campaign_id || data.resourceName || null;
        if (cid) { try { sessionStorage.setItem("sai_campaign_id", cid); } catch {} }
      }
  ...
  async function handleAiImprove(type, index) {
    ...
    try {
      ...
      form.append("type", type === "h" ? "headline" : "description");
      form.append("productTitle", selProduct?.title || "");
      const res  = await fetch("/app/api/ai-improve", { method: "POST", body: form });
      const data = await res.json();
  ...
  const handleUpgradeClick = React.useCallback(openUpgrade, [openUpgrade]);

  // ── findAiForProduct helper ───────────────────────────────────────────────
  function findAiForProduct(aiProducts, product) {
  ...
  if (hasScanAccess) {
    ...
    if (isPaid && analyzedCount === 0) return (
      <div className="sr dk"><style>{CSS}</style><div className="bg-m"/>
