...
const _action = async ({ request }: RouteHandlerArgs): Promise<Response> => {
  let session;
  try { ({ session } = await authenticate.admin(request)); } catch (authErr: unknown) { return Response.json({ success: false, error: "Auth failed" }, { status: 401 }); }
  ...
  const action = (formData.get("action") as string); // list, pause, enable, remove, diagnose
  ...
  try {
    ...
    switch (action) {
      ...
      case "pause": return Response.json(await updateCampaignStatus(token, customerId, (formData.get("campaignId") as string), "PAUSED"));
      case "enable": return Response.json(await updateCampaignStatus(token, customerId, (formData.get("campaignId") as string), "ENABLED"));
      case "remove": return Response.json(await updateCampaignStatus(token, customerId, (formData.get("campaignId") as string), "REMOVED"));
...
async function diagnoseCampaigns(token, customerId) {
  ...
  try {
    ...
    logger.warn("campaign-manage.recommendations", "Recommendations query failed", { extra: { error: (e as Error).message } });
  }
  ...
  try {
    ...
    logger.warn("campaign-manage.adPolicy", "Ad policy query failed", { extra: { error: (e as Error).message } });
  }
