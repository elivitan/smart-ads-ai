...
import { checkLicense, useScanCredit } from "../license.server";
...
async function saveAiResultsToDB(shop, products, aiProducts) {
  ...
  for (const aiProduct of aiProducts) {
    ...
    try {
      ...
      logger.error("scan.saveDB", `Failed to save AI analysis for ${productId}`, { error: (err as Error).message });
    }
  }
  return saved;
}
...
const _action = async ({ request }: RouteHandlerArgs): Promise<Response> => {
  ...
  try {
    ...
    logger.error("scan.action", "Auth failed", { error: (authErr as Error).message });
  ...
  try {
    ...
    if (step === "fetch") {
      ...
      for (const p of products) {
        ...
        } catch (err: unknown) { logger.warn("scan.fetch", "Failed to save product", { extra: { id: p.id, error: (err as Error).message } }); }
      }
    ...
    logger.error("scan.action", "Scan error", { shop, error: (err as Error).message });
    return Response.json({ success: false, error: (err as Error).message || "Scan failed" }, { status: 500 });
  }
};
