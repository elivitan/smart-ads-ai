...
// from SerpAPI, Google Ads API, and Shopify — and makes decisions
// like a senior ad agency campaign manager.
//
...
/**
 * SerpAPI: Search Google for real competitor data
 * Returns: ads running, organic competitors, shopping prices, trend data
 */
export async function collectCompetitorData(keywords, storeDomain) {
  if (!SERP_KEY) return { ads: [], organic: [], shopping: [], trends: null, competitorCount: 0 };
  ...
  for (const kw of keywords.slice(0, 3)) {
    try {
      const url = `https://serpapi.com/search.json?q=${encodeURIComponent(kw)}&api_key=${SERP_KEY}&num=20&hl=en&gl=us`;
      const res = await fetch(url, { signal: AbortSignal.timeout(10000) });
      if (!res.ok) continue;
      const data = await res.json();

      // Paid ads — who is spending money on this keyword
      if (data.ads) {
      ...
      // Organic results — who ranks naturally
      ...
      // Shopping results — real prices from competitors
      ...
      console.warn(`[AI-Brain] SerpAPI failed for "${kw}":`, err.message);
    }
  }
