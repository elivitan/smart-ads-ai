...
export const CONFIG: AppConfig = {
  ...
  healthScore: {
    ...
    curve: { power: 0.85 },
    decay: {
      fresh: { days: 1, factor: 1.0 },
      recent: { days: 7, factor: 0.95 },
      week: { days: 14, factor: 0.9 },
      month: { days: 30, factor: 0.8 },
      stale: { days: 60, factor: 0.6 },
      veryStale: { days: 90, factor: 0.4 },
    },
    confidence: {
      veryLow: { products: 2, factor: 0.5 },
      low: { products: 5, factor: 0.7 },
      medium: { products: 10, factor: 0.85 },
      high: { products: 20, factor: 0.95 },
      full: { products: 21, factor: 1.0 },
    },
    criticalPenaltyPerIssue: 8,
    maxCriticalPenalty: 30,
    industryBaseline: 62,
    grades: {
  ...
  livePulse: {
    targetFps: 24,
    defaultCpc: 0.44,
    pollIntervalLive: 30000,
    pollIntervalDemo: 2800,
  },
  launch: {
    pollInterval: 2000,
    pollMaxAttempts: 30,
  ...
  rateLimits: {
    aiCallsPerMinute: 10,
    googleAdsCallsPerMinute: 30,
    serpApiCallsPerDay: 100,
    campaignCreationsPerHour: 5,
  },
  features: {
    enablePMaxImages: true,
    enablePMaxVideos: false,
    enableCompetitorScraping: true,
    enableLaunchPolling: true,
    enableHealthDecay: true,
    enableNonLinearScoring: true,
    showConfidenceInHealthScore: true,
    showIndustryBaseline: true,
  },
};
