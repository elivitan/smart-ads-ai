...
// ══════════════════════════════════════════════
// Modals.tsx — OnboardModal + BuyCreditsModal
// Extracted from Index() to prevent remount on every re-render
// ══════════════════════════════════════════════
...
interface OnboardModalProps {
  onClose: () => void;
  onboardTab: "subscription" | "credits";
  setOnboardTab: (tab: "subscription" | "credits") => void;
  onboardStep: number;
  ...
  scanCredits: number;
  setScanCredits: (val: number) => void;
  onLaunchChoice: () => void;
}
...
const SCAN_PACKAGES: CreditPackage[] = [
...
export const OnboardModal = React.memo(function OnboardModal({
  ...
  setScanCredits,
  onLaunchChoice,
}: OnboardModalProps): React.JSX.Element {
  return (
      ...
      <div className="modal onboard-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="onboard-tabs">
          <button
            ...
            onClick={() => { setOnboardTab("subscription"); setOnboardStep(1); }}
          >
            📋 Subscription Plans
          </button>
          <button
        ...
        {onboardTab === "credits" ? (
          <div className="onboard-content">
            ...
            <p className="onboard-sub">
              No subscription needed. Each credit = one full product scan with competitor intelligence &amp; AI ad copy.
            </p>
            <div className="scan-credit-packages">
              {SCAN_PACKAGES.map((pkg, i) => (
                <div
                  key={i}
                  className={`scan-credit-pkg ${pkg.best ? "scp-popular" : ""}`}
                  onClick={() => { setScanCredits(scanCredits + pkg.amt); onClose(); }}
                >
            ...
            <div className="onboard-credits-tip">
              ...
              <span onClick={() => setOnboardTab("subscription")}>Compare subscription plans →</span>
            </div>
          </div>
        ) : onboardStep === 1 ? (
          <div className="onboard-content">
            ...
            <div className="plan-cards plan-cards-3">
              {PLANS.map((plan) => (
                ...
                >
                  {plan.badge && (
                    <div className={`plan-badge ${plan.badgeGold ? "plan-badge-gold" : ""}`}>
                      {plan.badge}
                    </div>
                  )}
            ...
            <button className="btn-onboard" disabled={!selectedPlan} onClick={() => setOnboardStep(2)}>
              Continue →
            </button>
          </div>
        ) : (
          <div className="onboard-content">
            ...
            <div className="google-connect-box">
              ...
              {googleConnected ? (
                ...
                <button className="btn-google" onClick={() => setTimeout(() => setGoogleConnected(true), 1500)}>
                  Connect Google Ads
                </button>
              )}
            ...
            {googleConnected && (
              <button className="btn-onboard" onClick={() => { onClose(); onLaunchChoice(); }}>
...
const AI_CREDIT_PACKAGES: AiCreditPackage[] = [
  { amt: 50, price: "$4.99" },
...
export const BuyCreditsModal = React.memo(function BuyCreditsModal({
  ...
  aiCredits,
  setAiCredits,
}: BuyCreditsModalProps): React.JSX.Element {
  return (
    <div className="credits-modal-overlay" onClick={onClose}>
      <div className="credits-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <p style={{ fontSize: 13, color: "rgba(255,255,255,.5)", marginBottom: 20 }}>
          Balance: <strong style={{ color: "#a5b4fc" }}>{aiCredits} credits</strong>
        </p>
        <div className="credits-packages">
          {AI_CREDIT_PACKAGES.map((pkg, i) => (
            <div
              key={i}
              className={`credit-pkg ${pkg.best ? "credit-pkg-popular" : ""}`}
              onClick={() => { setAiCredits(aiCredits + pkg.amt); onClose(); }}
            >
