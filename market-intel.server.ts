...
interface MarketIntelligenceResult extends MarketAnalysis {
  _raw: {
    holidays: UpcomingHoliday[];
    seasonal: SeasonalInsight;
    trends: { direction: string; trendChange: number; recentInterest: number } | null;
    newsCount: number;
    regions: string[];
    productCategory: string;
    analyzedAt: string;
...
async function getGoogleTrends(keywords: string[], region: string = "us"): Promise<TrendsResult | null> {
  ...
  try {
    ...
    const recentAvg = recent.reduce((sum: number, t: Record<string, unknown>) => {
      const val = (t.values as Array<{ extracted_value?: number }>)?.[0]?.extracted_value || 0;
      return sum + val;
    }, 0) / Math.max(recent.length, 1);

    const olderAvg = older.reduce((sum: number, t: Record<string, unknown>) => {
      const val = (t.values as Array<{ extracted_value?: number }>)?.[0]?.extracted_value || 0;
      return sum + val;
    ...
    return {
      ...
      direction: parseFloat(trendDirection) > 10 ? "rising" : parseFloat(trendDirection) < -10 ? "falling" : "stable",
      dataPoints: timeline.length,
    };
  } catch (err: unknown) {
    const message = err instanceof Error ? err.message : String(err);
    console.warn("[MarketIntel] Google Trends fetch failed:", message);
    return null;
  }
}
...
async function getMarketNews(regions: string[], productCategory: string): Promise<NewsResult | null> {
  ...
  try {
    ...
    for (const q of queries.slice(0, 2)) { // limit to 2 queries to save API calls
      ...
      const data = await res.json();
      if (data.articles) {
        allArticles.push(...data.articles.map((a: Record<string, unknown>) => ({
          title: a.title as string,
          description: ((a.description as string) || "").slice(0, 200),
    ...
    const message = err instanceof Error ? err.message : String(err);
    console.warn("[MarketIntel] NewsAPI fetch failed:", message);
    return null;
  }
}
...
async function analyzeWithClaude(context: AnalysisContext): Promise<MarketAnalysis> {
...
Respond ONLY with valid JSON.`;
  ...
  try {
    ...
    const message = err instanceof Error ? err.message : String(err);
    console.error("[MarketIntel] Claude analysis failed:", message);
    return {
      signal: "yellow",
...
export async function getMarketIntelligence(storeInfo: StoreInfo = {}): Promise<MarketIntelligenceResult> {
...
export async function getQuickMarketSignal(storeInfo: StoreInfo = {}): Promise<QuickSignalResult> {
  const regions = storeInfo.regions || ["US"];
  const productCategory = storeInfo.productCategory || "general";
  const month = new Date().getMonth() + 1;
